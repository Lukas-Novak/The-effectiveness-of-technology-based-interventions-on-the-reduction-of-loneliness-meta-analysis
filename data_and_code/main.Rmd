...
table_2_data <- data.frame(
    ...
    CI_lower   = round(meta_model$yi - qnorm(0.975) * sqrt(meta_model$vi), 3),  
    CI_upper   = round(meta_model$yi + qnorm(0.975) * sqrt(meta_model$vi), 3),  
    Z_value    = round(meta_model$yi / sqrt(meta_model$vi), 3),
    P_value    = round(2 * pnorm(abs(meta_model$yi / sqrt(meta_model$vi)),
                                 lower.tail = FALSE), 3)
)
