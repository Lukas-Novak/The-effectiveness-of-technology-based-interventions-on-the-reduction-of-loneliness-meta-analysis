...
cor_dat <-  readxl::read_xlsx("~/data_and_code/loneliness-interventions-meta-analysis/Data/pre_post_correlations.xlsx")
dat <- read.csv("~/data_and_code/loneliness-interventions-meta-analysis/Data/meta_data.csv")
...
```{r forest-plot}
# Create publication-ready labels for the plot
# This replaces the underscore with a space and adds parentheses around the year.
analysis_data$pretty_slab <- gsub("_", " (", analysis_data$slab)
analysis_data$pretty_slab <- paste0(analysis_data$pretty_slab, ")")

# Update the meta-analysis model from metafor to use the new labels for the plot
meta_model <- rma(
  yi, vi,
  data = analysis_data,
  method = "REML",
  test = "knha",
  slab = pretty_slab # Use the new, clean labels
)
...
# Add annotations
# We move the text up slightly to ensure it's visible

# Heterogeneity Statistics
text(x = -4.7, y = -1.5, pos = 4, cex = 0.8, bquote(paste("Heterogeneity: Q = ",
    .(round(meta_model$QE, 2)), ", df = ", .(meta_model$k - meta_model$p),
    ", p = ", .(round(meta_model$QEp, 3)), "; ", I^2, " = ",
    .(round(meta_model$I2, 1)), "%")))

# Prediction Interval (New values)
text(x = -4.7, y = -2.5, pos = 4, cex = 0.8, bquote(paste("Prediction Interval: [",
    .(round(pi_model$lpi, 2)), ", ",
    .(round(pi_model$upi, 2)), "]")))
